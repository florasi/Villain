...
class SyNet_client_mnist(nn.Module):
    def __init__(self,n_workers=2):
        ...
        size_1 = int(32*32/n_workers)
        size_2 = int(128/n_workers)
        self.lin1 = nn.Linear(size_1, 512)
        self.lin2 = nn.Linear(512, 256)
        self.lin3 = nn.Linear(256, 128)
        self.lin4 = nn.Linear(128, size_2)
...
class M_Discriminator(nn.Module):
    def __init__(self, embedding_len=64):
        super(M_Discriminator, self).__init__()
        self.lin1 = nn.Linear(embedding_len, 128)
...
class VGG(nn.Module):
    ...
    def __init__(self, block_nums,n_workers=2):
    ...
    def forward(self, x):
        x=F.interpolate(x.unsqueeze(0), mode='trilinear',size=(3,112,224),align_corners=True).squeeze(0)
        x = self.block1(x)
        x = self.block2(x)
        x = self.dropout(x)
        x = self.block3(x)
...
def SyNet_client_imagenette(n_workers=2):
    block_nums = [2, 2, 3, 3, 3]
    model = VGG(block_nums,n_workers)
    return model
...
class SyNet_client_cinic10(nn.Module):
    ...
    def __init__(self,net_name="VGG16",n_workers=2):
        ...
        self.classifier=nn.Sequential(
            ...
            nn.Linear(256,256),
            nn.ReLU(True),
            nn.Linear(256,int(128/n_workers))
            )
    ...
    def forward(self,x):
        x=F.interpolate(x.unsqueeze(0), mode='trilinear',size=(3,16,32),align_corners=True).squeeze(0)
        x=self.features(x)
        x=x.view(x.size(0),-1)
        x=self.classifier(x)
        return x
...
class SyNet_client_credit(nn.Module):
    ...
    def __init__(self,n_workers=2):
        ...
        size_1 = int(10/n_workers)
        size_2 = int(128/n_workers)
        self.fc1 = nn.Linear(size_1, 32)
        self.fc2 = nn.Linear(256, 256)
        self.fc3 = nn.Linear(256, 256)
        self.fc4 = nn.Linear(256, size_2)
